...
# Varijanta: SDNE embedding (autoencoder na adjacency; ovde linearna SVD aproksimacija)

# SDNE

"""
Graphs in Space: Graph Embeddings for Machine Learning on Complex Data
SDNE = structural deep embedding; ovde low-rank SVD nad matricom susedstva (red = ulaz čvora)
"""
...
def compute_sdne_embeddings(M, k=EMBED_DIM):
    A = M[1:40, 1:40].astype(float)
    X = A + np.eye(39)

    U, S, Vh = np.linalg.svd(X, full_matrices=False)
    n_comp = min(k, len(S))
    emb = U[:, :n_comp] * np.sqrt(np.maximum(S[:n_comp], 1e-12))
    if n_comp < k:
...
def main():
    ...
    emb = compute_sdne_embeddings(M, k=EMBED_DIM)
    ...
    print()
    print("Predikcija (SDNE + kvantna regresija, deterministički, strukturno):")
    print(best_combo)
    print()
    """
    Predikcija (SDNE + kvantna regresija, deterministički, strukturno):
    (8, 11, 23, 26, 33, 34, 35)
    """
...
X = A + I (matrica susedstva sa self-loop-ovima; 
svaki red = ulaz za jedan čvor, 39 redova).

SVD(X) → embedding = U[:, :k] * sqrt(S[:k]) 
(niska rang aproksimacija kao kod autoencoder-a).
